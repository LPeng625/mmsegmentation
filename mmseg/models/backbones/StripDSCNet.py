...
from mmseg.models.backbones.dscnetTools.S3_DSConv import DSConv
from mmseg.models.backbones.birenetTools.ffm import CBAM, SELayer, SpatialGate, ECAAP
...
class StripDSCNet(nn.Module):  # strip-unet
    def __init__(self, out_channels=2):
        super(StripDSCNet, self).__init__()
        ...
        self.DSCStripModuleConcat3 = DSCStripModuleConcat(filters[2])
        self.DSCStripModule3 = DSCStripModule(filters[2])
        self.stripatt = TDAtt(filters[2])
        ...
        self.finalconv3 = nn.Conv2d(32, out_channels, 3, padding=1)  # 1024*1
    ...
    def forward(self, x):
        ...
        # e3 = self.DSCStripModuleConcat3(e3)
        # e3 = self.DSCStripModule3(e3)
        e3 = self.stripatt(e3)
...
class DAM(nn.Module):
    def __init__(self, c_size, channel):
        ...
        self.GPA1 = nn.AdaptiveAvgPool2d((None, None))  # TODO 输入和输出大小一致
...
# TODO add
class DSCStripModule(nn.Module):  # 蛇形卷积+条带模块 一般般
    def __init__(self, dim):
        super(DSCStripModule, self).__init__()
        # DSCConv
        self.block1 = nn.Sequential(
            DSConv(dim, dim, kernel_size=3, extend_scope=3, morph=0, if_offset=True, device="cuda"),
            nn.BatchNorm2d(dim))
        self.block2 = nn.Sequential(
            DSConv(dim, dim, kernel_size=5, extend_scope=5, morph=1, if_offset=True, device="cuda"),
            nn.BatchNorm2d(dim))
        self.conv0 = nn.Conv2d(dim, dim, 5, padding=2, groups=dim)
        # StripConv
        self.conv0_1 = nn.Conv2d(dim, dim, (1, 7), padding=(0, 3), groups=dim)
        ...
        if self.FFM_has_SE:
            self.atten = SELayer(dim)

        if self.FFM_has_SAM:
            self.atten = SpatialGate()

        if self.FFM_has_ECA:
            self.atten = ECAAP(dim)
    ...
    def forward(self, x):
        ...
        attn_1 = self.conv1_1(attn)
        attn_1 = self.conv1_2(attn_1)
        attn_2 = nonlinearity(self.block1(attn))
        attn_3 = nonlinearity(self.block2(attn))
        x = x + attn_0 + attn_1 + attn_2 + attn_3
...
# TODO concat
class DSCStripModuleConcat(nn.Module):  # 蛇形卷积+条带模块 一般般
    def __init__(self, dim):
        super(DSCStripModuleConcat, self).__init__()
        # DSCConv
        self.ec0 = EncoderConv(dim, dim)
        self.dscconv1 = DSConv(dim, dim, kernel_size=3, extend_scope=1, morph=0, if_offset=True, device="cuda")
        self.dscconv2 = DSConv(dim, dim, kernel_size=3, extend_scope=1, morph=1, if_offset=True, device="cuda")
        # StripConv
        ...
        self.ec1 = EncoderConv(5 * dim, dim)
    ...
    def forward(self, x):
        ...
        x1 = self.dscconv1(x)
        x2 = self.dscconv2(x)
        x3 = self.conv0_2(self.conv0_1(x))
        x4 = self.conv1_2(self.conv1_1(x))
        x5 = self.ec1(torch.cat([x0, x1, x2, x3, x4], dim=1))
